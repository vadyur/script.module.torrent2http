...
class Engine:
	...
	def __init__(self, uri=None, binaries_path=None, platform=None, download_path=".",
					...
					dht_routers=None, trackers=None, buffer=0): # .005
		"""
		...
		:param buffer: Buffer percentage from start of file (default 0.005)
		"""
	...
	def file_status(self, file_index, timeout=10):
		...
		:rtype: FileStatus
		"""
		files = self._decode(self._request('ls', timeout))['file']
		if files:
			st = self.status(timeout)

			f = files[0]
			f['offset']		= 0
			f['download']	= st.total_download
			f['progress']	= st.progress
			res = FileStatus(index=file_index, media_type=self._detect_media_type(f['name']), **f)
		if res:
			return res
		else:
			raise Error("Requested file index (%d) is invalid" % file_index, Error.INVALID_FILE_INDEX, file_index=file_index)
